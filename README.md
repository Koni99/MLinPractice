...
- `-t`or `--tokenize`: Tokenize the given column (can be specified by `--tokenize_input`, default = "tweet"), and create new column "column_tokenized" containing tokenized tweet.
- `-l` or `--lowercase`: A new column "tweet_lowercased_and_tokenized" is created, where every word from "column_tokenized" is turned into lower case.
...
- '-st' or '--stopwords': Count the number of stopwords in the tokenized and lowercased tweet. (see code/feature_extraction/stopwords_counter.py)
- `-ha` or `--hashtags`: Count the number of hashtags in the "hashtags" column of the data frame. (see code/feature_extraction/hashtag_counter.py)
...
- '-u' or '--uniform': Uniform distribution classifier. Generates predictions uniformly at random.
- '-knn': K Nearest Neighbor classifier with hyperparameter k. Considers k closest training examples in data set.
